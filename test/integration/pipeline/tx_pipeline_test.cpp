...
#include "framework/integration_framework/utils.hpp"

using integration_framework::IntegrationTestFramework;
using iroha::protocol::TxStatus;

/**
 * Get the key for specified user
 * @param user who's key is being asked
 * @return keypair of that user
 */
iroha::keypair_t get_key_for(const std::string &user) {
  iroha::KeysManagerImpl manager(user);
  manager.createKeys("pass");
...
TEST(TransactionPipeline, SendTx) {
  ...
  integration_framework::IntegrationTestFramework()
      ...
      .addUser(user, key)
      .sendTx(addSignature(tx, key),
              [](auto status) {
                ASSERT_EQ(TxStatus::STATELESS_VALIDATION_SUCCESS, status);
              })
      .done();
}
